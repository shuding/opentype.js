...
describe.skip('bidi.js', function () {
    let latinFont;
    let arabicFont;
    let bidiFira;
    ...
    before(function () {
        ...
        bidiScheherazade.registerModifier('glyphIndex', null, (token) =>
            arabicFont.charToGlyphIndex(token.char)
        );
        ...
        bidiFira = new Bidi();
        bidiFira.registerModifier('glyphIndex', null, (token) =>
            latinFont.charToGlyphIndex(token.char)
        );
        const latinFeatures = [
            {
                script: 'latn',
    ...
    describe('arabic contexts', function () {
        ...
        it('should match mixed arabic sentence', function () {
            ...
            const sentences = ranges.map((range) =>
                arabicTokenizer.rangeToText(range)
            )[0];
    ...
    describe('applyFeatures', function () {
        it('should apply arabic presentation forms', function () {
            ...
            const ranges =
                bidiScheherazade.tokenizer.getContextRanges('arabicWord');
            const PeaceTokens = bidiScheherazade.tokenizer.getRangeTokens(
                ranges[1]
            );
